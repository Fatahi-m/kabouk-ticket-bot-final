...
if not TOKEN:
    # 🚨 مطمئن شوید که توکن در متغیرهای محیطی تنظیم شده است
    raise ValueError("TELEGRAM_BOT_TOKEN environment variable is required")

# 🚨 مقداردهی TELEGRAM_ADMIN_ID خود را با دقت بررسی کنید
...
LANGUAGES = {
    ...
    "ckb": { # کوردی سورانی (CKB)
        ...
        # 🆕 Neue Admin-Texte
        "clear_sales_prompt": "⚠️ *ئاگاداری!* ئایا دڵنیایت کە دەتەوێت *هەموو* داتای فرۆشتنەکان (بلیتەکان) بە شێوەیەکی نەگەڕێنراوە بکوژێنیتەوە؟ ئەم کردارە ناتوانرێت هەڵبگیرێتەوە!",
...
async def start(update: Update, context: CallbackContext):
    ...
    # 🚨 از متد get_user برای جلوگیری از خطاهای احتمالی در دسترسی به فیلدهای optional استفاده می‌کنیم
    try:
...
async def handle_callback_query(update: Update, context: CallbackContext):
    ...
    # اگر کاربر در دیتابیس نبود، ایجاد شود (بیشتر برای چت ادمین یا خطاهای اولیه)
    if not current_user:
        current_user = User(
            telegram_id=chat_id,
...
async def admin_sales_report(update: Update, context: CallbackContext):
    ...
    # استفاده از defaultdict برای گروه بندی بر اساس خریدار، رویداد و وضعیت
    sales_data = defaultdict(lambda: {'tickets_count': 0, 'status': None, 'issue_date': None, 'event_price': 0, 'user': None, 'event': None})

    # برای این که هر خرید یکبار شمرده شود، از ticket_id_str (که برای هر خرید جدید ایجاد می‌شود) استفاده می‌کنیم
    # اگر منطق شما در ثبت بلیط‌ها به این شکل است که همه بلیط‌های یک خرید از یک reference_code/ticket_id_str استفاده می‌کنند:

    # یک رویکرد تمیزتر برای گزارش: گروه‌بندی بر اساس `user_id`، `event_id` و `status`
    # اگر شما در کد بالا، برای تمام بلیط‌های یک خرید از `first_ticket_id_str` استفاده می‌کنید، باید آن را در اینجا گروه‌بندی کنیم.

    # فرض می‌کنیم اولین بلیط هر مجموعه pending/issued حاوی کد مرجع اصلی است.

    # ما از یک دیکشنری برای گروه بندی بر اساس (user_id, event_id, status) استفاده خواهیم کرد:
    grouped_sales = defaultdict(lambda: {'tickets_count': 0, 'status': None, 'issue_date': None, 'event_price': 0, 'user': None, 'event': None})

    # استفاده از یک ست برای جلوگیری از شمردن چندین بار یک سفارش
    ...
    for ticket in all_tickets:
        # برای گروه بندی، از user_id, event_id و status استفاده می‌کنیم.
        key = (ticket.user_id, ticket.event_id, ticket.status)

        # تنها اگر یک سطر جدید برای این کلید (ترکیب کاربر/رویداد/وضعیت) باشد
    ...
    # Sortierung nach Datum für bessere Lesbarkeit
...
async def admin_clear_sales(update: Update, context: CallbackContext):
    # این بررسی ادمین در اینجا اختیاری است، چون در Handler هم فیلتر اعمال شده، اما برای اطمینان بیشتر نگه داشته می‌شود.
    chat_id = update.effective_chat.chat_id
    # اگر این تابع بدون فیلتر ADMIN_ID در Handler ثبت شود، این خط امنیت را تضمین می‌کند
    if chat_id != ADMIN_ID:
        await update.message.reply_text(get_text("de", "not_authorized"))
        return

    # Erstelle Inline-Tastatur zur Bestätigung
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(get_text("de", "clear_sales_confirm_button"), callback_data="confirm_clear_sales")],
    ])

    await update.message.reply_text(
        get_text("de", "clear_sales_prompt"), # Zeigt die Warnmeldung an
...
def create_ticket(name, ticket_id_str, event_name):
    ...
    # Bestimme die Größe in mm, indem von Pixel (angenommen 96 DPI) zu Millimeter umgerechnet wird
...
if __name__ == '__main__':
    ...
    # --- ثبت Command Handlers (دستورات) ---
    app.add_handler(CommandHandler("start", start))

    # 🚨 دستورات ادمین همیشه باید در بالاترین اولویت باشند و با فیلتر دقیق ثبت شوند.
    # این اصلاح تضمین می‌کند که فیلتر چت ادمین به درستی اعمال شود.
    app.add_handler(CommandHandler("sales", admin_sales_report, filters=filters.Chat(ADMIN_ID)))
    app.add_handler(CommandHandler("clearsales", admin_clear_sales, filters=filters.Chat(ADMIN_ID)))

    # --- ثبت Message Handlers (پیام‌های عادی، عکس و فایل) ---
